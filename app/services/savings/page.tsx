...
export default function SaccoRegistrationGridPage() {
  ...
  const handleSaccoOnboarding = async (e: React.FormEvent) => {
    ...
    try {
      // EVOLUTION: Calling the unified App Router endpoint architecture 
      ...
      setStatus({ loading: false, msg: `🟢 ${data.message || "Institutional credentials logged! Pending statutory compliance audit."}`, error: false });
    } catch (err: any) {
      // PRESERVED: Your fallback simulation loop to support sandbox testing environments
      await new Promise(resolve => setTimeout(resolve, 350));
      setStatus({ 
        loading: false, 
        msg: `🔌 [SANDBOX CONFIRMATION] Corporate entity credentials logged for ${form.saccoLegalName || "Institution"}. Node socket initialized.`, 
        error: false 
      });
    }
  };

  // Reusable inline style sheets mimicking your existing palette config
  ...
  const inputStyle = { width: "100%", padding: "12px 16px", background: "#0f172a", border: "1px solid #334155", borderRadius: "8px", color: "#ffffff", fontSize: "14px", boxSizing: "border-box" as const };
  const formRowGridStyle = { display: "flex", gap: "20px", marginBottom: "20px" };
  const sectionDividerTitleStyle = { color: "#34d399", fontSize: "14px", fontWeight: "700", borderBottom: "1px solid #334155", paddingBottom: "6px", marginTop: "30px", marginBottom: "15px", textTransform: "uppercase" as const, letterSpacing: "0.5px" };
        ...
        <form onSubmit={handleSaccoOnboarding} style={{ background: "#1e293b", padding: "40px", borderRadius: "16px", width: "100%", maxWidth: "680px", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.5)", border: "1px solid #334155" }}>
          ...
          <div style={formRowGridStyle}>
            <div style={{ flex: 1 }}>
              ...
              <select value={form.settlementBankName} required onChange={e => setForm({...form, settlementBankName: e.target.value})} style={inputStyle}>
            ...
            <div style={{ flex: 1 }}>
              <label style={labelStyle}>Settlement Branch Name</label>
              <input type="text" placeholder="Kampala Main Branch" required value={form.settlementBranch} onChange={e => setForm({...form, settlementBranch: e.target.value})} style={inputStyle} />
            </div>
          </div>

          <div style={formRowGridStyle}>
            <div style={{ flex: 1 }}>
              <label style={labelStyle}>Settlement Corporate Account No.</label>
              <input type="text" placeholder="0140098765401" required value={form.settlementAccountNo} onChange={e => setForm({...form, settlementAccountNo: e.target.value})} style={inputStyle} />
            </div>
            <div style={{ flex: 1 }}>
              <label style={labelStyle}>Swift Routing Code (Optional)</label>
              <input type="text" placeholder="SBICUGKX" value={form.swiftCode} onChange={e => setForm({...form, swiftCode: e.target.value})} style={inputStyle} />
            </div>
          </div>
          ...
          <button 
            ...
            style={{ width: "100%", background: "#34d399", color: "#0f172a", padding: "14px", borderRadius: "8px", border: "none", fontSize: "15px", fontWeight: "bold", cursor: status.loading ? "not-allowed" : "pointer", marginTop: "10px", transition: "background 0.2s" }}
          >
            {status.loading ? "Processing Institutional Verification..." : "Verify & Onboard Sacco Infrastructure"}
          </button>
